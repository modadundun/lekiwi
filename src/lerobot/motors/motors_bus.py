...
class SerialMotorsBus(MotorsBusBase):
    ...
    def write(
        ...
        # --- DEBUG/DIAG: keep the pre-encode value ---
        pre_encode_value = value

        try:
            value = self._encode_sign(data_name, {id_: value})[id_]
        except ValueError as e:
            # 这里会把“哪个电机、写什么寄存器、编码前是什么值”一次性告诉你
            raise ValueError(
                f"{e}\n"
                f"[WRITE CONTEXT] motor='{motor}' id_={id_} model={model} data_name='{data_name}' "
                f"normalize={normalize} pre_encode_value={pre_encode_value}"
            ) from e
